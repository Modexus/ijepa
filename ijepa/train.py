...
from ijepa.helper import init_model, init_opt, load_checkpoint
...
from ijepa.utils.distributed import AllReduce, init_distributed
from ijepa.utils.logging import AverageMeter, CSVLogger, gpu_timer, grad_logger
...
def main(args, resume_preempt=False):
    ...
    dump = Path(folder) / "params-ijepa.yaml"
    ...
    log_file = Path(folder) / f"{tag}_r{rank}.csv"
    save_path = Path(folder) / f"{tag}" + "-ep{epoch}.pth.tar"
    latest_path = Path(folder) / f"{tag}-latest.pth.tar"
    load_path = None
    if load_model:
        load_path = (Path(folder) / r_file) if r_file is not None else latest_path
            ...
            def log_stats():
                ...
                if (itr % log_freq == 0) or np.isnan(loss) or np.isinf(loss):
                    ...
                    if grad_stats is not None:
                        logger.info(
                            f"[{epoch + 1}, {itr:5d}] "
                            f"grad_stats: [{grad_stats.first_layer:.2e} "
                            "{grad_stats.last_layer:.2e}] "
